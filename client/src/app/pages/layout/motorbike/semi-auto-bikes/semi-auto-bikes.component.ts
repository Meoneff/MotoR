...
@Component({
  selector: 'app-semi-auto-bikes',
  standalone: true,
  imports: [ShareModule, TaigaModule],
  templateUrl: './semi-auto-bikes.component.html',
  styleUrl: './semi-auto-bikes.component.scss',
})
export class SemiAutoBikesComponent implements OnInit, OnDestroy {
  index = 0;
  ...
  semi$ = this.store.select('motor', 'motorGetByCategoryId');
  ...
  constructor(
    ...
    this.store.dispatch(CategoryActions.getById({ categoryId: 'S01' }));
    this.store.dispatch(
      MotorActions.getMotorByCategoryId({
        categoryId: '6625357eaf10759eba05ccf2',
      }),
    );
  ...
  ngOnInit() {
    this.subscriptions.push(
      this.semi$.subscribe((semiCategories) => {
        if (semiCategories.length > 0) {
          console.log(semiCategories);
          //coppy lại sử dụng array sort
          this.motorList = [...semiCategories];
        }
      }),
    );
  }
